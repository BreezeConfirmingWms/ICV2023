    ...
    while Flag:
        ...
        if case_zhName == "25.停-走功能":
            ...
            if GetObstacle is not None:
                ...
                slowCar_velX = GetObstacle.contents.obstacle[0].velX
                slowCar_velY = GetObstacle.contents.obstacle[0].velY

            control_upd.gear = ESimOne_Gear_Mode(1)
            if SoGetGps(mainVehicleID, gps_data) and (slowCar_x - current_x) < 28 and (gps_data.velX - slowCar_velX) > 0:
                control_upd.gear = ESimOne_Gear_Mode(1)
                control_upd.brake = 0.25
                # control_upd.throttle = 0
            if SoGetGps(mainVehicleID, gps_data) and (slowCar_x - current_x) > 32 and (gps_data.velX - slowCar_velX) < 0:
                control_upd.gear = ESimOne_Gear_Mode(1)
                control_upd.throttle = 0.25
                # control_upd.brake = 0
            if SoGetGps(mainVehicleID, gps_data) and abs(gps_data.velX - slowCar_velX) < 0.1:
                control_upd.brake = 0
                control_upd.throttle = 0
