"""
@Time : 2023/10/13 20:03  
@Author : BreezeConfirming
@Software : Pycharm
@File : NewWpAndTrajDemo.py
...
traj_data=SimOne_Data_Trajectory()


traj_data.trajectorySize=5
traj_data.trajectory[0].posX=1.5
traj_data.trajectory[0].posY=3.2
traj_data.trajectory[0].vel=10/3.6

traj_data.trajectory[1].posX=1.5
traj_data.trajectory[1].posY=5.2
traj_data.trajectory[1].vel=10/3.6

traj_data.trajectory[2].posX=1.4
traj_data.trajectory[2].posY=5.8
traj_data.trajectory[2].vel=3/3.6

traj_data.trajectory[3].posX=1.5
traj_data.trajectory[3].posY=7.2
traj_data.trajectory[3].vel=3/3.6

traj_data.trajectory[4].posX=1.6
traj_data.trajectory[4].posY=10.5
traj_data.trajectory[4].vel=3/3.6
...
if __name__ == '__main__':
    ...
    while Flag:
        ...
        #if case_zhName == "41.连续测试场景（赛道)":
        if case_zhName == "06.车道居中控制":



            # if SoGetWayPoints(mainVehicleID,waypointData):
            #     for i in range(waypointData.wayPointsSize):
            #
            #         if waypointData.wayPoints[i].posX !=0 :
            #             print(waypointData.wayPoints[i].posX)
            #         if waypointData.wayPoints[i].posX !=0 :
            #             print(waypointData.wayPoints[i].posY)
            #         if waypointData.wayPoints[i].posX !=0 :
            #             print(waypointData.wayPoints[i].heading_x)



            if SoSetDriveTrajectory(mainVehicleID,traj_data):
                if cnt%3==1:
                    print("trajectory regulation success!!!")
            #
