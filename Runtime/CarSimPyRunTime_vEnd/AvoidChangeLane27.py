    ...
    while Flag:
        ...
        if case_zhName == "27.避让事故车辆变道":
            ...
            if GetObstacle is not None:
                for i in range(0,GetObstacle.contents.obstacleSize):
                    if GetObstacle.contents.obstacle[i].posY > most_Left_car_Y:
                        most_Left_car_Y = GetObstacle.contents.obstacle[i].posY
                    if GetObstacle.contents.obstacle[i].posX < most_Near_car_X:
                        most_Near_car_X = GetObstacle.contents.obstacle[i].posX
                if cnt % 5 == 0:
