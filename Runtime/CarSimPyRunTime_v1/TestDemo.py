...
if __name__ == '__main__':
    ...
    obstacle_Data = SimOne_Data_Obstacle()
    ...
    while Flag:
        ...
        potentialObstacleInde = obstacle_Data.obstacleSize
        ...
        if SoGetGroundTruth(mainVehicleID, obstacle_Data):
            if cnt % 2 == 0:
                print(obstacle_Data.obstacleSize)
                print("Obstacle one's position is: \
                X:{0}, Y:{1}, Z:{2}".format(obstacle_Data.obstacle[0].posX,
                obstacle_Data.obstacle[0].posY,
                obstacle_Data.obstacle[0].posX))
        else:
            print("Get Obstacle Failed!")
